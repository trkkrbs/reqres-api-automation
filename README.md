...
## ⚙️ Postman Ortam Kurulumu (Environment Setup)

Projeyi içe aktardıktan sonra isteklerin (özellikle `{{baseUrl}}` hatası almamak için) çalışması için bir Postman ortamı oluşturmanız gerekmektedir:

1.  Postman'de sol menüden **Environments** sekmesine gidin.
2.  **+ (Create Environment)** butonuna tıklayın.
3.  Ortam adını `ReqRes-Config` olarak belirleyin.
4.  Aşağıdaki tabloya göre değişkenleri (**Variables**) ekleyin:

| Variable | Initial Value |
| :--- | :--- |
| **baseUrl** | `https://reqres.in` |
| **api_key** | *(Aşağıdaki adıma bakın)* |
| **userId** | *(Boş bırakılabilir)* |
| **token** | *(Boş bırakılabilir)* |
| **time now** | *(Boş bırakılabilir)* |

> **💡 Not:** `userId`, `token` ve `time now` gibi değerler testler koşturuldukça scriptler tarafından otomatik olarak doldurulacaktır.

---

## 🔑 Ücretsiz API Key Alma

Günlük **250 ücretsiz istek** sınırından yararlanmak ve testleri kısıtlama olmadan koşturmak için kendi API anahtarınızı tanımlamanız önerilir:

1.  [app.reqres.in/api-keys](https://app.reqres.in/api-keys) adresine gidin.
2.  Sayfadaki adımları takip ederek ücretsiz hesabınızı oluşturun ve size özel **API Key**'inizi alın.
3.  Aldığınız bu anahtarı, oluşturduğunuz ortamdaki **api_key** değişkeninin karşısına (**Initial Value** kısmına) yapıştırın.
