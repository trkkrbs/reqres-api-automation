...
⚙️ Postman Ortam Kurulumu (Environment Setup)Projeyi içe aktardıktan sonra isteklerin çalışması için bir Postman ortamı (Environment) oluşturmanız gerekmektedir:Postman'de sol menüden Environments sekmesine gidin ve + (Create Environment) butonuna tıklayın.Ortam adını ReqRes-Config olarak belirleyin.Aşağıdaki tabloya göre değişkenleri (Variables) ekleyin:VariableInitial ValuebaseUrlhttps://reqres.inapi_key(Aşağıdaki adıma bakın)userId(Boş bırakılabilir)token(Boş bırakılabilir)time now(Boş bırakılabilir)Not: userId, token ve time now gibi değerler testler koşturuldukça otomatik olarak doldurulacaktır.🔑 Ücretsiz API Key AlmaGünlük 250 ücretsiz istek sınırından yararlanmak için kendi API anahtarınızı tanımlamanız önerilir:app.reqres.in/api-keys adresine gidin.Sayfadaki adımları takip ederek ücretsiz hesabınızı oluşturun ve size özel API Key'inizi alın.Aldığınız bu anahtarı, oluşturduğunuz Postman ortamındaki api_key değişkeninin karşısına (Initial Value kısmına) yapıştırın.Yapmamı istediğin başka bir ekleme var mı? Örneğin, bu ayarları yaptıktan sonra testleri nasıl koşturacaklarını anlatan kısa bir "Usage" bölümü de ekleyebiliriz.
